...
// ── Config & state ────────────────────────────────────────
// All values load from environment variables on startup so they
// survive Render restarts. Set these in Render → Environment.
let config = {
  venueUrl:       process.env.VENUE_URL      || '',
  cookies:        process.env.COOKIE_STRING  || '',
  ntfyTopic:      process.env.NTFY_TOPIC     || '',
  preferredTimes: process.env.PREFERRED_TIMES ? process.env.PREFERRED_TIMES.split(',').map(s => s.trim()) : [],
  timeFrom:       process.env.TIME_FROM      || '',
  timeTo:         process.env.TIME_TO        || '',
  courts:         process.env.COURTS         ? process.env.COURTS.split(',').map(s => s.trim()) : [],
  daysAhead:      parseInt(process.env.DAYS_AHEAD    || '7'),
  intervalMin:    parseInt(process.env.INTERVAL_MIN  || '10'),
  enabled:        process.env.AUTO_START === 'true'
};
...
app.listen(PORT, () => {
  ...
  // Log what was loaded from environment
  if (config.cookies)    console.log('[env] COOKIE_STRING loaded (' + config.cookies.length + ' chars)');
  if (config.ntfyTopic)  console.log('[env] NTFY_TOPIC: ' + config.ntfyTopic);
  if (config.venueUrl)   console.log('[env] VENUE_URL: ' + config.venueUrl);
  if (config.enabled) {
    console.log('[env] AUTO_START=true — starting monitor…');
    startMonitor();
  }
});
